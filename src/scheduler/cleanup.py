...
async def cleanup_stale_data_job():
    ...
    job_stats.start_run("cleanup_stale_data")
    ...
    try:
        ...
        job_stats.finish_run("cleanup_stale_data", processed=cleaned_count)

    except Exception as e:
        logger.error(f"Cleanup job failed: {e}")
        job_stats.finish_run("cleanup_stale_data", error=str(e))
